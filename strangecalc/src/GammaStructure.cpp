...
GammaStructure& GammaStructure::add(const GammaStructure& left, const GammaStructure& right)
{
  int *sumcomp = new int[16];
  complex<double> *sumcompCoeff = new complex<double>[16];
  ...
  for(int i=0; i<left.nrComp; i++)
    for(int j=0; j<right.nrComp; j++)
      sumcompCoeff[ multTable[left.comp[i]][right.comp[j]] ] +=
	left.compCoeff[i] * right.compCoeff[j] *
	multTableCoeff[left.comp[i]][right.comp[j]];
...
GammaStructure& GammaStructure::add(const complex<double>& factor, const GammaStructure& right)
{
  int *sumcomp = new int[16];
  complex<double> *sumcompCoeff = new complex<double>[16];
  ...
  for(int i=0; i<right.nrComp; i++)
      sumcompCoeff[right.comp[i]] += factor * right.compCoeff[i];
...
//! Multiplication of GammaStructure * double
...
//! Multiplication of GammaStructure * complex
...
//! Multiplication of GammaStructure * GammaStructure
GammaStructure GammaStructure::operator*(const GammaStructure& right) const 
{
...
//! Multiplication of double * GammaStructure
...
//! Multiplication of complex * GammaStructure
...
//! Evaluates the structure and returns the 4x4 matrix
...
void GammaStructure::print() const 
{
  cout << *this << endl;
}
...
//! Parses a complex value with an underflow (=1e-14)
complex<double> cleanComplex(const complex<double>& x)
{
  const double underflow = 1.e-14;

  return complex<double>( std::fabs(x.real()) < underflow ? 0. : x.real(),
			  std::fabs(x.imag()) < underflow ? 0. : x.imag());
}
...
ostream& operator<<(ostream& output, const GammaStructure& gamma) 
{
  ...
  for(int i=0; i<15; i++) {
    if(gamma.comp[index] == i) {
      output << cleanComplex(gamma.compCoeff[index]);
