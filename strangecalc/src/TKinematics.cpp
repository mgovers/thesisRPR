 ...
 *
 * See http://rprmodel.ugent.be/api/root/TKinematics.html
 *
 * \author Pieter Vancraeyveld <pieter.vancraeyveld@ugent.be>
 * \author Tom Vrancx <tom.vrancx@ugent.be>
 * 
...
//_____________________________________________________________________
double TKinematics::GetTMax() const
{
...
//_____________________________________________________________________
double TKinematics::GetUMin() const
{
  // Returns the minimum value of -u.

  return fMy*fMy - fQsquared -2.0*fWcm*sqrt(fPk*fPk+fMy*fMy) + 2.0*fPk*fKcm;
}

//_____________________________________________________________________
double TKinematics::GetUMax() const
{
  // Returns the maximum value of -u.

  return fMy*fMy - fQsquared -2.0*fWcm*sqrt(fPk*fPk+fMy*fMy) - 2.0*fPk*fKcm;
}

//_____________________________________________________________________
double TKinematics::GetWMin() const
{
  // Returns the minimum value of w.

  return fMk + fMy;
}

//_____________________________________________________________________
double TKinematics::GetWlabMin() const
{
  // Returns the minimum value of wlab.

  return (GetWMin()*GetWMin() + fQsquared - fMp*fMp)/2./fMp;
}

//_____________________________________________________________________
double TKinematics::GetWcmMin() const
{
  // Returns the minimum value of wcm.

  return (GetWMin()*GetWMin() - fQsquared - fMp*fMp)/2./GetWMin();
}

//_____________________________________________________________________
double TKinematics::GetElectroBeamEnergyMin() const
{
  // Returns the minimum value of electron beam energy

  return (fWlab + sqrt(fWlab*fWlab + fQsquared))/2.;
}
...
void TKinematics::UpdateKinematics()
{
  ...
  if( fIsospin==11 || fIsospin==12) { // radiative capture
    ...
    else if(compareDoubleUFlow(ek, fMk)) {
      fIsPhysical = true;
      fPk = 0;
    }
    else {
    ...
    if( fAVar == &fCosthkcm ) {
      ...
      if(compareDoubleUFlow(fT, GetTMin()))
	fCosthkcm = 1;
      else if(compareDoubleUFlow(fT, GetTMax()))
	fCosthkcm = -1;
      else
	fCosthkcm = ( fT + fQsquared + 2.0*fWcm*sqrt(fPk*fPk+fMk*fMk) - fMk*fMk )
      ...
      if(compareDoubleUFlow(fU, GetUMin()))
	fCosthkcm = -1;
      else if(compareDoubleUFlow(fU, GetUMax()))
	fCosthkcm = 1;
      else
	fCosthkcm = ( fT + fQsquared + 2.0*fWcm*sqrt(fPk*fPk+fMk*fMk) - fMk*fMk )
	  / 2.0 / fPk / fKcm;
    ...
    if( std::fabs(fCosthkcm) <= 1.0 )
      fIsPhysical = true && fIsPhysical;
    else if(compareDoubleUFlow(std::fabs(fCosthkcm), 1)) {
      fIsPhysical = true && fIsPhysical;
      fCosthkcm = ( fCosthkcm>0.0 ? 1.0 : -1.0 );
    }
    else
